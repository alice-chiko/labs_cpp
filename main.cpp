...
int main() {
    ...
    auto dataset = DataProcessor::generateDataset(200, k, b);
    ...
    auto [newX, newY] = MathOps::transformVector(transform, oldX, oldY);
    
    Logger::logValue("Original X", oldX);
    Logger::logValue("Transformed X", newX);

    float totalLoss = Metrics::totalAbsoluteError(dataset, k, b);
    Logger::logValue("Final Absolute Loss", totalLoss);

    if (DataProcessor::saveToCSV(dataset, "points.csv")) {
