...
int main() {
    ...
    // Генерируем данные с шумом (noise_level = 5.0)
    ...
    // Обучаем перцептрон только на train_data
    NeuralNet::Perceptron<float> model(0.01f);
    model.train(train_data, 1000);

    // Проверяем точность на test_data 
    int correct = 0;
    ...
    //  Сохраняем для визуализации (возвращаем масштаб 100)
    for (auto& pt : full_dataset) {
        pt.x *= 100.0f;
        pt.y *= 100.0f;
    }
